...
@pytest.fixture
def app_instance():
    """Provides a AhrefsApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return AhrefsApp(integration=mock_integration)

def test_universal_mcp_ahrefs_app_initialization(app_instance):
    """
    Test that the AhrefsApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "ahrefs", "AhrefsApp instance has unexpected name."


def test_universal_mcp_ahrefs_tool_docstrings_format(app_instance):
    """
    Test that each tool method in AhrefsApp has a well-formatted docstring,
...
def test_universal_mcp_ahrefs_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in AhrefsApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
